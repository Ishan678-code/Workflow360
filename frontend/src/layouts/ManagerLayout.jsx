...
const navItems = [
  ...
  { to: "/manager/leaves",      label: "Leave Approvals",  icon: CalendarCheck },
  { to: "/manager/employees",   label: "Employees",        icon: Users },
  { to: "/manager/performance", label: "Performance",      icon: Star },
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFF] flex flex-col font-sans selection:bg-violet-600 selection:text-white">
      ...
      <div
        ...
        style={{ backgroundImage: `radial-gradient(#6d28d9 1px, transparent 1px)`, backgroundSize: "24px 24px" }}
      />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex-1 flex items-center">
            <div className="flex items-center gap-3 group cursor-pointer">
              <div className="w-9 h-9 bg-violet-700 rounded-xl flex items-center justify-center shadow-lg shadow-violet-200 group-hover:scale-105 transition-all">
                <span className="text-white font-black text-sm">W</span>
              </div>
              <div className="hidden xl:flex flex-col">
                <span className="text-slate-900 font-black text-[14px] leading-tight tracking-tight">Workforce360</span>
                <span className="text-[10px] text-violet-600 font-bold uppercase tracking-wider">Manager Portal</span>
              </div>
            </div>
          </div>
            ...
            {navItems.map((item) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  ...
                  ${isActive
                    ? "bg-white text-violet-600 shadow-sm ring-1 ring-slate-200/50"
      ...
      <main className="flex-1 relative z-10 max-w-7xl mx-auto w-full px-4 sm:px-6 py-8">
        <div className="mb-8 flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">
          ...
          <span className="text-violet-600">Active Session</span>
        </div>
