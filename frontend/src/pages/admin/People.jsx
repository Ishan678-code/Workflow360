    ...
    try {
      const payload = matchForm.projectId
        ...
        : {
            ...
            requiredSkills: matchForm.requiredSkills
              .split(",")
              .map((s) => s.trim())
              .filter(Boolean),
          };
            ...
            {matchResults.length > 0 && (
              <div className="mt-6 space-y-3">
                ...
                {matchResults.map((result, idx) => {
                  const f = result.freelancer || result;
                  const score = typeof result.score === "number" ? result.score : null;
                  const breakdown = result.breakdown || null;
                  return (
                    <article key={f._id || idx} className="rounded-2xl bg-slate-50 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-100 text-xs font-black text-rose-700">
                            {getInitials(f.name || "FL")}
                          </div>
                          <div>
                            <p className="text-sm font-black text-slate-900">#{idx + 1} {f.name || "Freelancer"}</p>
                            <p className="text-xs text-slate-500">{Array.isArray(f.skills) ? f.skills.join(", ") : f.role || "—"}</p>
                          </div>
                        </div>
                        {score !== null && (
                      ...
                      {breakdown && (
                        <div className="mt-3 flex gap-4 text-[11px] text-slate-500">
                          <span>TF-IDF: <strong className="text-slate-700">{(breakdown.tfidf * 100).toFixed(0)}%</strong></span>
                          <span>Skills: <strong className="text-slate-700">{(breakdown.skillOverlap * 100).toFixed(0)}%</strong></span>
                          <span>Coverage: <strong className="text-slate-700">{(breakdown.tokenCoverage * 100).toFixed(0)}%</strong></span>
                        </div>
                      )}
                    </article>
                  );
